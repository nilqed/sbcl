...
(defun %print-debugger-invocation-reason (condition stream)
  (princ (code-char 2) stream)
  (princ "verbatim:" stream)
  (format stream "~2&")
    ...
    (handler-case
      ...
      (error (condition)
        ...
        (let ((ndc-type (type-of *nested-debug-condition*)))
          (princ (code-char 2) *error-output*)
          (princ "verbatim:" *error-output*)
          (format *error-output*
                  ...
                  (princ (code-char 5) *error-output*)
                  (princ (code-char 10) *error-output*))
...
(defun debug-loop-fun ()
  (let* ((*debug-command-level* (1+ *debug-command-level*))
    ...
    (handler-bind ((sb-di:debug-condition
                    (lambda (condition)
                      (progn
                        (princ (code-char 2) *debug-io*)
                        (princ "verbatim:" *debug-io*)
                        (princ condition *debug-io*)
                        (princ (code-char 5) *debug-io*)
                        (princ (code-char 10) *debug-io*))
  ...
  (let ((num (read-if-available :prompt)))
    ...
    (let ((restart (typecase num
      ...
      (if restart
          ...
          (progn
            (princ (code-char 2) *debug-io*)
            (princ "verbatim:There is no such restart." *debug-io*)
            (princ (code-char 5) *debug-io*)
            (princ (code-char 10) *debug-io*))
          ))))

